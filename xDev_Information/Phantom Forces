...
if isfolder and makefolder and not isfolder('xDeveloper_PF') then
    makefolder('xDeveloper_PF')
    DarkHub_Client.SaveSettings = true
    if isfile('xDeveloper_PF/xDeveloper_PF'..tostring(Game.PlaceId)..'.xDeveloper_PF') then
        DarkHub_Client.SavedSettings = true
        DarkHub_Client.GameSettings = game:GetService('HttpService'):JSONDecode(readfile('xDeveloper_PF/xDeveloper_PF'..tostring(Game.PlaceId)..'.xDeveloper_PF'))
    end
elseif isfolder('xDeveloper_PF') then
    DarkHub_Client.SaveSettings = true
    if isfile('xDeveloper_PF/xDeveloper_PF'..tostring(Game.PlaceId)..'.xDeveloper_PF') then
        DarkHub_Client.SavedSettings = true
        DarkHub_Client.GameSettings = game:GetService('HttpService'):JSONDecode(readfile('xDeveloper_PF/xDeveloper_PF'..tostring(Game.PlaceId)..'.xDeveloper_PF'))
...
if not IsUpToDate and DarkHub_Client.SavedSettings and isfile('xDeveloper_PF/xDeveloper_PF'..tostring(Game.PlaceId)..'.xDeveloper_PF') then
    delfile('xDeveloper_PF/xDeveloper_PF'..tostring(Game.PlaceId)..'.xDeveloper_PF')
    game.Players.LocalPlayer:Kick('xDev Settings Out Of Date - Issue Fixed | Restart xDev to Continue')
end

local lib = loadstring(game:HttpGet("https://raw.githubusercontent.com/xDeveloping/xDeveloping_UI-Functions/main/xDev_UI/xDev_UI_Dark_V3.lua",true))()
...
local Credit = main:Tab('Credit')


Credit:Label("●——————————◦◉◦——————————●")
Credit:Button(". . . . . ╰──╮ Discord Sever ╭──╯ . . . . .", function() 
local a=request or http_request or syn and syn.request;local b=game.HttpService;a({Url="http://127.0.0.1:6463/rpc?v=1",Method="POST",Headers={["Content-Type"]="application/json",["Origin"]="https://discord.com"},Body=b:JSONEncode({cmd="INVITE_BROWSER",args={code="gcXvFDbvJ9"},nonce=b:GenerateGUID(false)})})local c=game:GetService("Players").LocalPlayer
end)

Credit:Button(". . . . . ╰──╮ Discord [ Copy ] ╭──╯ . . . . .", function()
    setclipboard("https://discord.com/users/612489304391811092")
end)
 
Credit:Button("  . . . . . ╰──╮ Facebook [ Copy ] ╭──╯ . . . . .",function()
    setclipboard("https://www.facebook.com/profile.php?id=100069125066175")
end)
Credit:Label("●——————————◦◉◦——————————●")
